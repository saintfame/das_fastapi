...
__all__ = ["IncomingBook", "ReturnedAllBooks", "ReturnedBook", "ReturnedBookSeller"]
...
class IncomingBook(BaseBook):
    ...
    def validate_year(val: int):
        if not(datetime.now().year >= val > 1900):
...
class ReturnedBookSeller(BaseBook):
    id: int
    count_pages: int
...
class ReturnedBookSellerList(BaseModel):
